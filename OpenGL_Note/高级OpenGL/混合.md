...
`glBlendFunc(GLenum sfactor, GLenum dfactor)`函数接受两个参数，来设置==源==和==目标因子==。OpenGL为我们定义了很多个选项，我们将在下面列出大部分最常用的选项。注意常数颜色向量$\vec{C}_{constant}$可以通过`glBlendColor`函数来另外设置。
